...
class ProtoLoss(nn.Module):
    ...
    def forward(self, mu_s: torch.Tensor, f_t: torch.Tensor) -> torch.Tensor:
        ...
        s_dist = F.softmax(sim_mat/self.nav_t, dim=0)     # modification 1 
        t_dist = s_dist/(s_dist.sum(1).reshape(len(s_dist), 1))
        cost_mat = self.pairwise_cosine_dist(mu_s, f_t)
        # cost_mat = -torch.log(F.softmax(sim_mat, dim=0))   # modification 2 
        source_loss = (self.s_par*cost_mat*s_dist).sum(0).mean() 
        # target_loss = (((1-self.s_par)*cost_mat*t_dist).sum(1)*self.prop.squeeze(1)).sum()
        # target_loss = (1-self.s_par)*((s_dist*cost_mat).sum(1).reshape(len(s_dist),1) * s_dist).sum(0).mean() # modification 3 
        target_loss = (1-self.s_par)*((t_dist*cost_mat).sum(1) * s_dist.mean(1)).sum() # modification 3 
