...
class ImageList(datasets.VisionDataset):
    ...
    def __init__(self, root: str, classes: List[str], data_list_file: str, 
            transform: Optional[Callable] = None, target_transform: Optional[Callable] = None, subsample: Optional[str] = 'all'):
        ...
        #     self.labels_to_idx = self.get_labels_to_idx(self.data)
        self.data = self.subsample(self.data, self.labels_to_idx, subsample)
        self.labels_to_idx = self.get_labels_to_idx(self.data)
        ...
        # print(len(self.data))
        # print(f'Size of datasets: {len(self.data):d}')  # modification
    
    def subsample(self, data, labels_to_idx, sub_sam: str):
        np.random.seed(0)
        keep_idx = set()
        ...
        for label in sorted(labels_to_idx.keys()):
            ...
            train_idx = set(np.random.choice(labels_to_idx[label], int(0.8*len(labels_to_idx[label])), replace=False).tolist())
            val_test_idx = set(labels_to_idx[label]) - train_idx
            val_idx = set(np.random.choice(list(val_test_idx), int(0.5*len(val_test_idx)), replace=False).tolist())
            test_idx = val_test_idx - val_idx 
            if sub_sam == 'all': 
                keep_idx.update(set(labels_to_idx[label]))
            elif sub_sam == 'train': 
                keep_idx.update(train_idx)
            elif sub_sam == 'val': 
                keep_idx.update(val_idx)
            elif sub_sam == 'test': 
                keep_idx.update(test_idx)
            else: 
                raise Exception('Subsample name error! Only \'all\', \'train\', \'val\', or \'test\'. ')
